function initMap() {
    ...
    postos.forEach(function (posto) {
        ...
        markers.push({ id: posto.id, marker: marker });

        var infoWindowContent = '<div>' +
            '<h2>' + posto.nome + '</h2>' +
            '<p><strong>Endereço:</strong> ' + posto.endereco + '</p>' +
            '<button style="background-color: red; color: white; height: 30px; width: 30px" onclick="window.location.href=\'' + posto.site + '\'">Ir</button>' +
            '</div>';
    ...
    window.toggleStatus = function (id) {
        var markerData = markers.find(m => m.id === id);
        if (markerData) {
            var currentIcon = markerData.marker.getIcon();
            var newIcon = currentIcon.url.includes("red-dot") ?
    ...
    document.getElementById('toggleStatus').addEventListener('click', function () {
        ...
        // Alterna o estado global (se todos estavam verdes, eles ficam alternados e vice-versa)
        allMarkersGreen = !allMarkersGreen;
    });
}
